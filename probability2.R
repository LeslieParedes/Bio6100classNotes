...
my_histo <- function(x=NULL,data_type="cont") {
  if(is.null(x)) x=runif(1000) #if the user don't give any data then insert runif
  df <- data.frame(x=x)
...
my_pdf <- function(x=NULL,y=NULL,data_type="cont") {
  if(is.null(x) | is.null(y)) {
    x=seq(from=-3,to=3,length.out=100) #if x and y are missing lets build the data
    y=dnorm(x) }
...
my_pdf()
my_x=seq(from=0,to=1,length.out=100) #normal distribution
...
nbi_ran <- rnbinom(n=1000,size=0.1,mu=5) #the smallest parameter(size) the largest variance
my_histo(x=nbi_ran,data_type="disc")
...
summary(my_norm)
toss_zeroes <- my_norm[my_norm>0] ###vector that subsetting those elements that are greater than 0
...
# small sample sizes, uniform distribution
...
###goodness of fit test for data,find maximum likelihood estimators for the paramaters.
library(MASS)
data <- c(100,100,104,99)
z <- fitdistr(data,"normal") #normal distribution
print(z) #estimate parameters (mean and sd)
#example
...
x <- 1:100
g_density <- dnorm(x=x,mean=z$estimate["mean"],sd=z$estimate["sd"]) #from the fitdistr function
qplot(x,g_density,geom="line") + annotate(geom="point",x=frog_data,y=0.001) #density plot
...
###now let’s add a weird outlier of a tiny value and see what happens
frog_data <- c(30.15,26.3,27.5,22.9,27.8,26.2)
outlier <- 0.050
